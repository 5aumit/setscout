    ...
    for c in candidates:
        haystack = " ".join(
            [
                ...
                c.id,
                c.url,
                str(c.raw_metadata.get("repo_id", "")),
                str(c.raw_metadata.get("ref", "")),
            ]
        ).lower()
...
def node_searcher(state: SetScoutState) -> dict:
    ...
    max_candidates = int(os.environ.get("SETSCOUT_MAX_CANDIDATES", DEFAULT_MAX_CANDIDATES))
    capped = filtered[:max_candidates]
    logs = search_logs + [
        f"searcher: {len(filtered)} candidates after exclusions",
        f"searcher: {len(capped)} candidates selected for enrichment",
    ]
    return {"candidates": capped, "logs": logs}
